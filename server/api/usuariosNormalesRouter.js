...
router.get('/:registro', (req, res) => {
  db.query('SELECT * FROM usuariosnormales WHERE Registro = ?', [req.params.registro], (error, results) => {
    if (error) {
      console.error("Error encontrado: ", error);
      return res.status(500).json({ message: "Error al obtener la información." });
    }
...
router.post('/', (req, res) => {
  ...
  db.query(
    ...
    (error, results) => {
      ...
      res.status(201).json({ message: "Usuario creado correctamente", Registro: results.insertId });
    }
  );
});
...
router.put('/:registro', (req, res) => {
  const { Nombre, ApellidoPaterno, ApellidoMaterno, Celular, Correo, Tiempo, Nivel } = req.body;
  db.query(
    'UPDATE usuariosnormales SET Nombre = ?, ApellidoPaterno = ?, ApellidoMaterno = ?, Celular = ?, Correo = ?, Tiempo = ?, Nivel = ? WHERE Registro = ?',
    [Nombre, ApellidoPaterno, ApellidoMaterno, Celular, Correo, Tiempo, Nivel, req.params.registro],
    (error, results) => {
      if (error) {
        console.error("Error encontrado: ", error);
...
router.delete('/:registro', (req, res) => {
  db.query('DELETE FROM usuariosnormales WHERE Registro = ?', [req.params.registro], (error, results) => {
    if (error) {
      console.error("Error encontrado: ", error);

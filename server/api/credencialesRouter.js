...
router.get('/:id', (req, res) => {
  db.query('SELECT * FROM credenciales WHERE Id = ?', [req.params.id], (error, results) => {
    if (error) {
      console.error("Error encontrado: ", error);
      return res.status(500).json({ message: "Error al obtener la información." });
    }
...
router.post('/', async (req, res) => {
  ...
  db.query('INSERT INTO credenciales (Usuario, Contrasena) VALUES (?, ?)', [Usuario, hashedPassword], (error, results) => {
    if (error) {
      console.error("Error encontrado: ", error);
      return res.status(500).json({ message: "Error al crear la credencial." });
    }
    res.status(201).json({message: "Credencial añadida correctamente", id: results.insertId});
  });
});
...
router.put('/:id', async (req, res) => {
  ...
  db.query(
    ...
    (error, results) => {
      if (error) {
        console.error("Error encontrado: ", error);
        return res.status(500).json({ message: "Error al actualizar la credencial." });
      }
...
router.delete('/:id', (req, res) => {
  db.query('DELETE FROM credenciales WHERE Id = ?', [req.params.id], (error, results) => {
    if (error) {
      console.error("Error encontrado: ", error);
      return res.status(500).json({ message: "Error al eliminar la credencial." });
    }
